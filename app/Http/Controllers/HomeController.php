...
class HomeController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        $data = Cache::remember('homepage_data', 600, fn () => [
            ...
            'hero_qaris' => Qari::where('status', 'active')->whereNotNull('image')->withCount(['tilawat' => fn ($q) => $q->where('status', 'active')])->ordered()->take(12)->get(),
            'top_qaris' => Qari::where('status', 'active')->withCount(['tilawat' => fn ($q) => $q->where('status', 'active')])->ordered()->take(12)->get(),

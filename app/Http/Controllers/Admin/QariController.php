...
class QariController extends Controller
{
    ...
    public function store(StoreQariRequest $request)
    {
        ...
        if (Qari::where('slug', $slug)->exists()) {
            $slug .= '-'.Str::random(4);
        }
        Qari::create(
            [
    ...
    {
        $updates = ['name_ar' => $request->name_ar, 'name_en' => $request->name_en, 'biography_ar' => $request->biography_ar ?? null, 'biography_en' => $request->biography_en ?? null, 'status' => $request->status, 'is_featured' => $request->boolean('is_featured'), 'sort_order' => (int) $request->input('sort_order', $qari->sort_order)];
        if ($request->filled('image_tmp')) {
            if ($qari->image) {
                Storage::disk('public')->delete($qari->image);
            }

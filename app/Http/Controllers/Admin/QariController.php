...
class QariController extends Controller {
    ...
    public function create() { return view('admin.qaris.create'); }
    ...
    public function edit(Qari $qari) { return view('admin.qaris.edit',compact('qari')); }
    public function update(Request $request, Qari $qari) {
        ...
        if ($request->hasFile('image')) { if($qari->image)Storage::disk('public')->delete($qari->image); $updates['image']=$request->file('image')->store('qari-images','public'); }
        $qari->update($updates);
        Cache::forget('homepage_data');
    ...
    public function destroy(Qari $qari) {
        ...
        $qari->delete(); Cache::forget('homepage_data');

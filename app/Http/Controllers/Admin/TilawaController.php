...
class TilawaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (Tilawa::where('slug', $slug)->exists()) $slug .= '-' . Str::random(4);
        $dur = 0;
        try {
            $o = shell_exec('ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 ' . escapeshellarg($request->file('audio')->getRealPath()) . ' 2>/dev/null');
            if ($o && is_numeric(trim($o))) $dur = (int)round((float)trim($o));
        } catch (\Throwable) {
        }
    ...
    public function update(Request $request, Tilawa $tilawa)
    {
        ...
        if ($request->hasFile('audio')) {
            ...
            $u['audio_path'] = $request->file('audio')->store('tilawat', 'public');
            try {
                $o = shell_exec('ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 ' . escapeshellarg($request->file('audio')->getRealPath()) . ' 2>/dev/null');
                if ($o && is_numeric(trim($o))) $u['duration'] = (int)round((float)trim($o));
            } catch (\Throwable) {
            }
        }
        if ($request->hasFile('cover_image')) {
            if ($tilawa->cover_image) Storage::disk('public')->delete($tilawa->cover_image);
            $u['cover_image'] = $request->file('cover_image')->store('tilawa-covers', 'public');
        }
        $tilawa->update($u);
        Cache::forget('homepage_data');

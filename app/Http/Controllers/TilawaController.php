...
class TilawaController extends Controller
{
    public function show(Tilawa $tilawa)
    {
        ...
        $liked = auth()->check() ? Like::where('user_id', auth()->id())->where('tilawa_id', $tilawa->id)->exists() : false;
        $saved = auth()->check() ? SavedTilawa::where('user_id', auth()->id())->where('tilawa_id', $tilawa->id)->exists() : false;

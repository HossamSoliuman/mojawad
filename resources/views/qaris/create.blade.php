...
@section('content')
    ...
    <div class="max-w-xl mx-auto">
        ...
        <form method="POST" action="{{ route('qaris.store') }}" enctype="multipart/form-data"
            ...
            <div>
                ...
                <textarea name="bio" rows="4"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">{{ old('bio') }}</textarea>
                @error('bio')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <button type="submit"
                class="w-full bg-emerald-600 text-white py-2 rounded-lg text-sm font-medium hover:bg-emerald-700">Create
                Qari</button>
        </form>
    </div>

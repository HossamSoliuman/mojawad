...
<a href="{{ route('qaris.show', $qari) }}" class="bg-white rounded-xl border border-gray-200 p-4 text-center hover:shadow-md transition-shadow">
<div class="w-12 h-12 bg-emerald-100 rounded-full flex items-center justify-center mx-auto mb-2 overflow-hidden">
@if($qari->photo)
<img src="{{ Storage::url($qari->photo) }}" class="w-full h-full object-cover">
@else
<span class="text-emerald-700 font-bold text-lg">{{ mb_substr($qari->name_en, 0, 1) }}</span>
@endif
</div>
...
<a href="{{ route('creator.apply.create') }}" class="inline-block mt-2 text-sm text-emerald-600 hover:underline font-medium">Apply to become a Creator</a>
</div>
@endif
@endauth

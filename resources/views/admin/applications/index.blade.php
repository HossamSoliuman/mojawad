...
@csrf
<button type="submit" class="w-full bg-emerald-600 text-white text-xs px-4 py-2 rounded-lg hover:bg-emerald-700">Approve</button>
</form>
<form method="POST" action="{{ route('admin.applications.reject', $application) }}">
@csrf
<button type="submit" class="w-full bg-red-50 text-red-600 text-xs px-4 py-2 rounded-lg border border-red-200 hover:bg-red-100">Reject</button>
</form>
</div>
</div>
@endforeach

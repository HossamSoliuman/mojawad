...
<nav class="navbar z1">
  ...
  <div class="nav-links">
    <a href="{{ route('home') }}"        class="nav-link {{ request()->routeIs('home') ? 'active':'' }}"><i class="fas fa-house"></i> Home</a>
    <a href="{{ route('qaris.index') }}" class="nav-link {{ request()->routeIs('qaris.*') ? 'active':'' }}"><i class="fas fa-microphone"></i> Qaris</a>
    @auth
    <a href="{{ route('library') }}"    class="nav-link {{ request()->routeIs('library') ? 'active':'' }}"><i class="fas fa-bookmark"></i> Library</a>
    @endauth
  </div>
  <div class="navbar-right">
    ...
    <div class="search-wrap" x-data="searchBar()">
      ...
      <input class="search-input" type="text" placeholder="Search…"
             x-model="q" @input.debounce.300ms="search()" @keydown.escape="close()"
             @click.away="close()">
      <template x-if="q">
      ...
      <div class="search-dropdown" x-show="open" x-transition>
        ...
        <template x-if="results.tilawat && results.tilawat.length">
          <div>
            <div class="search-sec-hd" style="border-top:1px solid var(--border)"><i class="fas fa-music"></i> Tilawat</div>
    ...
    <div x-data="{ open:false }" style="position:relative">
      <button class="user-pill" @click="open=!open" @click.away="open=false">
        <img src="{{ auth()->user()->avatar_url }}" class="avatar" width="23" height="23" alt="">
      ...
      <div class="dropdown" x-show="open" x-transition>
        ...
        <a href="{{ route('admin.dashboard') }}"><i class="fas fa-gauge" style="color:var(--gold);width:15px"></i> Dashboard</a>
        @endif
        <a href="{{ route('profile') }}"><i class="fas fa-user" style="width:15px"></i> Profile</a>
        <form method="POST" action="{{ route('logout') }}">@csrf
          <button type="submit"><i class="fas fa-arrow-right-from-bracket" style="color:var(--red);width:15px"></i> Logout</button>
        </form>
      </div>
    </div>
    @else
    <a href="{{ route('login') }}"    class="btn btn-ghost btn-sm"><i class="fas fa-arrow-right-to-bracket"></i> Login</a>
...
<div class="player-bar hidden z1" id="playerBar" x-data="audioPlayer()">
  ...
  <div class="p-vol">
    <button class="p-btn" @click="muted=!muted;audio.muted=muted">
      <i class="fas" :class="muted?'fa-volume-xmark':(vol>0.5?'fa-volume-high':'fa-volume-low')"></i>
    </button>
    <input type="range" class="vol-slider" min="0" max="1" step="0.02"
           x-model="vol" @input="audio.volume=vol">
  </div>
</div>

<script src="https://cdn.jsdelivr.net/npm/alpinejs@3.x.x/dist/cdn.min.js" defer></script>
<script>
// Global play function called by play buttons
...
document.addEventListener('alpine:init', () => {
  Alpine.data('audioPlayer', () => ({
    audio: null, playing: false, cur: 0, dur: 0, pct: 0,
    vol: 1, muted: false,
    init() {
  ...
  Alpine.data('searchBar', () => ({
    ...
    async search() {
      if (this.q.length < 2) { this.close(); return; }
